...
in_hach <- Sys.glob(paths="./Sensor logs/Hach/sc1000_dl_21.10.21.csv")
in_n2o <- Sys.glob(paths="./Sensor logs/Unisense/n2o_211021.xlsx")
...
Merging data and plotting - 10/20/21
```{r}
# data cleaning
# filter hach data by cycle time
cycle_hach <- split_hach$'2021-10-20'  %>%
  filter(date_time >= as.POSIXct("2021-10-20 08:20:00", tz="GMT") & date_time <= as.POSIXct("2021-10-20 15:00:00", tz="GMT"))

cycle_n2o <- split_n2o$'2021-10-20'  %>%
  filter(date_time >= as.POSIXct("2021-10-20 08:20:00", tz="GMT") & date_time <= as.POSIXct("2021-10-20 15:00:00", tz="GMT"))
...
start_time <- as.POSIXct("2021-10-20 08:22:00", tz="GMT")

# time value
cycle_long$time_diff <- as.numeric(difftime(cycle_long$date_time, start_time, units="hours"))

# specify what to plot
params_to_plot <- c("NO3_mgNL","OP_mgPL","NO2_mgNL")
...
ggplot(data=subset(cycle_long, parameter %in% params_to_plot), mapping=aes(time_diff,value)) + 
  ...
  ylim(0,ylim_val) + 
  ggtitle("In-cycle nutrient sampling 10/20/21") +
  ...
  scale_color_cycle() + 
  scale_shape_manual(values=shape_values,labels=shape_labels)
ggsave("cycle_211020.png", height=4, width=7, units="in", scale=0.8)
```
