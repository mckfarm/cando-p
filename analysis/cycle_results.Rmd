...
shape_labels = setNames(c("NO2 mgNL","NO2+NO3 mgNL","NO3 mgNL","NH3 mgNL","OP mgPL",
                          "DO mgO2L","pH","ORP mV","N2O mgNL"),                          c("NO2_mgNL","NO2+NO3_mgNL","NO3_mgNL","NH3_mgNL","OP_mgPL",
                           "do_ppm","ph","orp","N2O_mgNL"))
...
in_hach <- Sys.glob(paths="./Sensor logs/Hach/hach_dl_21.11.18.csv")
in_n2o <- Sys.glob(paths="./Sensor logs/Unisense/n2o_211118.xlsx")
...
Merging data and plotting - 11.17.21
```{r}
# data cleaning
# filter hach data by cycle time
cycle_hach <- split_hach$'2021-11-17'  %>%
  filter(date_time >= as.POSIXct("2021-11-17 08:20:00", tz="GMT") & date_time <= as.POSIXct("2021-11-17 14:40:00", tz="GMT"))

cycle_n2o <- split_n2o$'2021-11-17'  %>%
  filter(date_time >= as.POSIXct("2021-11-17 08:20:00", tz="GMT") & date_time <= as.POSIXct("2021-11-17 14:40:00", tz="GMT"))
...
start_time <- as.POSIXct("2021-11-17 08:22:00", tz="GMT")
...
ggplot(data=subset(cycle_long, parameter %in% params_to_plot), mapping=aes(time_diff,value)) + 
  ...
  ylim(0,ylim_val) + 
  ggtitle("In-cycle nutrient sampling 11/17/21") +
...
ggsave("cycle_211117.png", height=4, width=7, units="in", scale=0.8)
```
