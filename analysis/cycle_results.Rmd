...
in_hach <- Sys.glob(paths="./Sensor logs/Hach/hach_dl_21.11.04.csv")
in_n2o <- Sys.glob(paths="./Sensor logs/Unisense/n2o_211104.xlsx")
...
Merging data and plotting - 11.04.21
```{r}
# data cleaning
# filter hach data by cycle time
cycle_hach <- split_hach$'2021-11-03'  %>%
  filter(date_time >= as.POSIXct("2021-11-03 08:20:00", tz="GMT") & date_time <= as.POSIXct("2021-11-03 14:40:00", tz="GMT"))

cycle_n2o <- split_n2o$'2021-11-03'  %>%
  filter(date_time >= as.POSIXct("2021-11-03 08:20:00", tz="GMT") & date_time <= as.POSIXct("2021-11-03 14:40:00", tz="GMT"))
...
start_time <- as.POSIXct("2021-11-03 08:22:00", tz="GMT")
...
ggplot(data=subset(cycle_long, parameter %in% params_to_plot), mapping=aes(time_diff,value)) + 
  ...
  ylim(0,ylim_val) + 
  ggtitle("In-cycle nutrient sampling 11/03/21") +
...
ggsave("cycle_211103.png", height=4, width=7, units="in", scale=0.8)
```
