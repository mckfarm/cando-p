...
data_n2o <- bind_rows(lapply(in_n2o,read_excel))
...
data_n2o <- data_n2o %>% 
  mutate(N2O_mgNL=rollapply(N2O_mgNL_temp,60,FUN=mean,partial=TRUE))
...
ggplot(data=subset(cycle_long, parameter=="orp"), mapping=aes(time_diff,value)) + 
  geom_point(aes(col=parameter, shape=parameter)) +
  xlab("Hours") + 
  ylab("ORP mV") +
  ggtitle("In-cycle nutrient sampling - ORP - 8/10/21") +
  geom_vline(xintercept=1.78, color="grey") + 
  geom_vline(xintercept=5.28, color="grey") + 
  ...
  scale_color_cycle() + 
  scale_shape_manual(values=shape_values,labels=shape_labels)
ggsave("cycle_210810_orp.png", height=4, width=7, units="in", scale=0.8)
```
