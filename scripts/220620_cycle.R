...
nutrients <- ggplot(data=cycle,aes(x=hour,y=value)) +
  ...
  theme_half_open() +
  ylim(0,40) +
  labs(y="Concentration [mgP/L or mgN/L]",x="Hour",title="220620 in-cycle sampling")  +
...
ggsave(filename= path_out %>% file.path("220620_cycle.png"),
...
ggplot(data=n2o,aes(x=hour,y=N2O_mgNL_raw)) +
  ...
  geom_vline(xintercept=1.65,color="grey") +
  geom_vline(xintercept=4.13,color="grey") +
  labs(y="N2O [mgN/L]",x="Hour",title="220620 N2O in-cycle sampling")
ggsave(filename= path_out %>% file.path("220620_n2o.png"),
       width=7,height=4,dpi=300,units="in")
